...
def get_balance(user_id: int, currency_id: int) -> int:
    """ユーザーの残高を取得します。"""
    ...
...
def transfer(source: int, dest: int, currency: int, amount: int) -> Transaction:
    """
    新しい送金を開始します。
...
def search_transactions(source: Optional[int] = None, dest: Optional[int] = None, currency: Optional[int] = None, page: int = 1) -> List[Transaction]:
    """トランザクションを検索します。"""
    ...
...
def create_claim(claimant: int, payer: int, currency: int, amount: int, desc: Optional[str] = None) -> Claim:
    """新しい請求を作成します。"""
    ...
...
def pay_claim(claim_id: int, payer_id: int) -> Transaction:
    """請求に対して支払います。"""
    ...

def cancel_claim(claim_id: int, user_id: int) -> Claim:
    """請求をキャンセルします。"""
    ...
