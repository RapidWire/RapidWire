...
class ContractAPI:
    ...
    async def get_balance(self, user_id: int, currency_id: int) -> int:
        balance = await self.core.get_user(user_id).get_balance(currency_id)
        return balance.amount

    async def transfer(self, source: int, dest: int, currency: int, amount: int) -> Transfer:
        if source != self.ctx.contract_owner_id:
            raise PermissionError("Contract can only initiate transfers from its own account.")
        return await self.core.transfer(source, dest, currency, amount, execution_id=self.ctx.execution_id)

    async def approve(self, spender: int, currency: int, amount: int):
        await self.core.approve(self.ctx.contract_owner_id, spender, currency, amount, execution_id=self.ctx.execution_id)

    async def transfer_from(self, sender: int, recipient: int, currency: int, amount: int) -> Transfer:
        return await self.core.transfer_from(sender, recipient, currency, amount, self.ctx.contract_owner_id, execution_id=self.ctx.execution_id)

    async def get_allowance(self, owner: int, spender: int, currency: int) -> int:
    ...
    async def get_transaction(self, tx_id: int) -> Optional[Transfer]:
        return await self.core.Transfers.get(tx_id)

    async def get_currency(self, currency_id: int) -> Optional[Currency]:
        curr = await self.core.Currencies.get(currency_id=currency_id)
        return curr if curr else None

    async def create_claim(self, claimant: int, payer: int, currency: int, amount: int, desc: Optional[str] = None) -> Claim:
        claim = await self.core.Claims.create(claimant, payer, currency, amount, desc)
        return claim

    async def get_claim(self, claim_id: int) -> Optional[Claim]:
        claim = await self.core.Claims.get(claim_id)
        return claim if claim else None
    
    async def pay_claim(self, claim_id: int, payer_id: int) -> Transfer:
        tx = await self.core.pay_claim(claim_id, payer_id)
        return tx

    async def cancel_claim(self, claim_id: int, user_id: int) -> Claim:
        claim = await self.core.cancel_claim(claim_id, user_id)
        return claim
    ...
    async def execute_contract(self, destination_id: int, input_data: Optional[str] = None) -> Optional[str]:
        ...
        callee_contract = await self.core.Contracts.get(destination_id)
        ...
        execution_id, output_data = await self.core.execute_contract(
            caller_id=source_id,
    ...
    async def get_variable(self, user_id: int|None, key: str) -> int | str | None:
        if user_id is None:
            user_id = self.ctx.contract_owner_id
        variable = await self.core.ContractVariables.get(user_id, key)
        return variable.value if variable else None

    async def set_variable(self, key: str, value: int | str):
        if len(key) > 31:
        ...
        user_variables = await self.core.ContractVariables.get_all_for_user(self.ctx.contract_owner_id)
        ...
        await self.core.ContractVariables.set(self.ctx.contract_owner_id, key, value)
    ...
    async def discord_send(self, guild_id: int, channel_id: int, message: str) -> bool:
        ...
        # Whitelist check
        if not await self.core.DiscordPermissions.check(guild_id, self.ctx.contract_owner_id):
        ...
        try:
            ...
            resp = await self.httpx_client.get(f"/channels/{channel_id}")
            ...
            resp = await self.httpx_client.post(f"/channels/{channel_id}/messages", json=payload)
    ...
    async def discord_role_add(self, guild_id: int, user_id: int, role_id: int) -> bool:
        ...
        try:
            ...
            resp = await self.httpx_client.put(url)
    ...
    async def has_role(self, guild_id: int, user_id: int, role_id: int) -> bool:
        if not self.core.Config.Discord.token:
            return False
...
class RapidWire:
    ...
    async def initialize(self):
        ...
        self.db = DatabaseConnection(self.pool)
    ...
    async def execute_contract(self, caller_id: int, contract_owner_id: int, input_data: Optional[str] = None, chain_context: Optional[ChainContext] = None) -> Tuple[int, str | None]:
        ...
        try:
            async with self.db as cursor:
                ...
                contract = await self.Contracts.get(contract_owner_id)
                if not contract or not contract.script:
                    raise ContractError("Contract not found or script is empty.")

                # Create Execution Record (Pending)
                execution_id = await self.Executions.create(cursor, caller_id, contract_owner_id, input_data, 'pending')
        ...
        try:
            ...
            async with self.db as cursor:
                ...
                await self.Executions.update(cursor, execution_id, output_data, chain_context.total_cost, 'success')
    ...
    async def create_currency(self, guild_id: int, name: str, symbol: str, supply: int, issuer_id: int, hourly_interest_rate: int) -> Tuple[Currency, Optional[Transfer]]:
        ...
        new_currency = await self.Currencies.create(guild_id, name, symbol, 0, issuer_id, hourly_interest_rate)

        initial_tx = None
        if supply > 0:
            initial_tx = await self.transfer(
        ...
        return await self.Currencies.get(guild_id), initial_tx

    async def renounce_currency(self, currency_id: int, user_id: int) -> Currency:
        currency = await self.Currencies.get(currency_id)
        if not currency:
        ...
        return await self.Currencies.renounce_minting(currency_id)

    async def mint_currency(self, currency_id: int, amount: int, minter_id: int) -> Transfer:
        currency = await self.Currencies.get(currency_id)
        if not currency:
            raise CurrencyNotFound("Currency not found.")
        ...
        return await self.transfer(SYSTEM_USER_ID, minter_id, currency_id, amount)

    async def burn_currency(self, currency_id: int, amount: int, burner_id: int) -> Transfer:
        return await self.transfer(burner_id, SYSTEM_USER_ID, currency_id, amount)

    async def request_delete_currency(self, currency_id: int, user_id: int) -> Currency:
        currency = await self.Currencies.get(currency_id)
        if not currency:
            raise CurrencyNotFound("Currency not found.")
    ...
    async def finalize_delete_currency(self, currency_id: int, user_id: int) -> List[Transfer]:
        ...
        if time_since_request > ten_days:
            await self.cancel_delete_request(currency_id)
    ...
    async def delete_currency(self, currency_id: int) -> List[Transfer]:
        ...
        for holder in holders:
            tx = await self.transfer(holder.user_id, SYSTEM_USER_ID, currency_id, holder.amount)
            transactions.append(tx)

        await self.Currencies.delete(currency_id)
        return transactions

    async def cancel_delete_request(self, currency_id: int):
        await self.Currencies.cancel_delete_request(currency_id)
    ...
    async def pay_claim(self, claim_id: int, payer_id: int) -> Transfer:
        try:
            async with self.db as cursor:
                ...
                await self.Claims.update_status(claim_id, 'paid')
            return tx
        except aiomysql.Error as err:
    ...
    async def cancel_claim(self, claim_id: int, user_id: int) -> Claim:
        ...
        return await self.Claims.update_status(claim_id, 'canceled')

    async def stake_deposit(self, user_id: int, currency_id: int, amount: int) -> Stake:
        if amount <= 0:
        ...
        try:
            async with self.db as cursor:
                ...
                await self.get_user(user_id)._update_balance(cursor, currency_id, -amount)

                # Upsert the stake
                await self.Stakes.upsert(cursor, user_id, currency_id, amount, int(time()))

                # Create a transfer for the deposit
                await self.Transfers.create(cursor, user_id, SYSTEM_USER_ID, currency_id, amount)
            return await self.Stakes.get(user_id, currency_id)
        except aiomysql.Error as err:
            raise TransactionError(f"Database error during stake deposit: {err}")

    async def stake_withdraw(self, user_id: int, currency_id: int, amount: int) -> Transfer:
        if amount <= 0:
        ...
        try:
            async with self.db as cursor:
                ...
                if new_stake_amount > 0:
                    await self.Stakes.update_amount(cursor, user_id, currency_id, new_stake_amount, int(time()))
                else:
                    await self.Stakes.delete(cursor, user_id, currency_id)

                # Transfer funds back to the user
                await self.get_user(user_id)._update_balance(cursor, currency_id, amount)

                # Create a transfer for the withdrawal
                transfer_id = await self.Transfers.create(cursor, SYSTEM_USER_ID, user_id, currency_id, amount)
            return await self.Transfers.get(transfer_id)
        except aiomysql.Error as err:
            raise TransactionError(f"Database error during stake withdrawal: {err}")

    async def request_interest_rate_change(self, currency_id: int, new_rate: int, user_id: int) -> Currency:
        currency = await self.Currencies.get(currency_id)
        if not currency:
            raise CurrencyNotFound("Currency not found.")
        if currency.issuer_id != user_id:
        ...
        return await self.Currencies.request_rate_change(currency_id, new_rate)

    async def apply_interest_rate_change(self, currency_id: int, user_id: int) -> Currency:
        currency = await self.Currencies.get(currency_id)
        if not currency:
            raise CurrencyNotFound("Currency not found.")
        ...
        return await self.Currencies.apply_rate_change(currency)

    async def set_contract(self, user_id: int, script: str, max_cost: Optional[int] = None, lock_hours: Optional[int] = None):
        if "\\" in script:
        ...
        current_contract = await self.Contracts.get(user_id)
        current_time = int(time())
        new_locked_until = 0
        ...
        try:
            async with self.db as cursor:
                execution_id = await self.Executions.create(cursor, user_id, 0, 'update_contract', 'pending')
                contract = await self.Contracts.set(user_id, script, cost, max_cost, new_locked_until)
                await self.ContractHistories.create(cursor, execution_id, user_id, script_hash, cost)
    ...
    async def approve(self, owner_id: int, spender_id: int, currency_id: int, amount: int, execution_id: Optional[int] = None):
        try:
            async with self.db as cursor:
                await self.Allowances.upsert(cursor, owner_id, spender_id, currency_id, amount)
                await self.AllowanceLogs.create(cursor, owner_id, spender_id, currency_id, amount, execution_id)
        except aiomysql.Error as err:
    ...
    async def create_liquidity_pool(self, currency_a_id: int, currency_b_id: int, amount_a: int, amount_b: int, user_id: int) -> LiquidityPool:
        if amount_a <= 0 or amount_b <= 0:
            raise ValueError("Amounts must be positive.")

        if await self.LiquidityPools.get_by_currency_pair(currency_a_id, currency_b_id):
        ...
        try:
            async with self.db as cursor:
                ...
                pool = await self.LiquidityPools.create(currency_a_id, currency_b_id, amount_a, amount_b, initial_shares)
                await self.LiquidityProviders.add_shares(cursor, pool.pool_id, user_id, initial_shares)
            return pool
        except aiomysql.Error as err:
    ...
    async def add_liquidity(self, symbol_a: str, symbol_b: str, amount_a_desired: int, amount_b_desired: int, user_id: int) -> Tuple[int, int]:
        ...
        try:
            async with self.db as cursor:
                ...
                await self.LiquidityPools.update_reserves(cursor, pool.pool_id, amount_a, amount_b, shares_to_mint)
                await self.LiquidityProviders.add_shares(cursor, pool.pool_id, user_id, shares_to_mint)

                await self.Transfers.create(cursor, user_id, SYSTEM_USER_ID, pool.currency_a_id, amount_a)
                await self.Transfers.create(cursor, user_id, SYSTEM_USER_ID, pool.currency_b_id, amount_b)
    ...
    async def remove_liquidity(self, symbol_a: str, symbol_b: str, shares: int, user_id: int) -> Tuple[int, int]:
        ...
        provider = await self.LiquidityProviders.get_by_pool_and_user(pool.pool_id, user_id)
        ...
        try:
            async with self.db as cursor:
                ...
                await user._update_balance(cursor, pool.currency_a_id, amount_a)
                await user._update_balance(cursor, pool.currency_b_id, amount_b)
                await self.LiquidityPools.update_reserves(cursor, pool.pool_id, -amount_a, -amount_b, -shares)

                new_shares = provider.shares - shares
                if new_shares > 0:
                    await self.LiquidityProviders.update_shares(cursor, pool.pool_id, user_id, new_shares)
                else:
                    await self.LiquidityProviders.delete(cursor, pool.pool_id, user_id)

                await self.Transfers.create(cursor, SYSTEM_USER_ID, user_id, pool.currency_a_id, amount_a)
                await self.Transfers.create(cursor, SYSTEM_USER_ID, user_id, pool.currency_b_id, amount_b)
            return amount_a, amount_b
        except aiomysql.Error as err:
        ...
        try:
            async with self.db as cursor:
                ...
                for i, pool in enumerate(locked_route):
                    ...
                    await self.LiquidityPools.update_reserves(cursor, pool.pool_id, reserve_a_change, reserve_b_change, 0)
