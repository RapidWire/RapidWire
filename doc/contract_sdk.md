...
コントラクトは、トランザクションを受信したとき（送金など）や、`/contract execute`コマンドで明示的に呼び出されたときに実行されます。
...
- **`sender`** (`int`): トランザクションの送信者（またはコントラクトの呼び出し元）のユーザーID。
- **`self_id`** (`int`): コントラクト所有者（あなた）のユーザーID。
- **`input_data`** (`str`): トランザクションまたは実行時に渡された入力データ。
