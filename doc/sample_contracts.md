...
これらの例は、単なる機能テストではなく、実際の運用で役立つ「実用的で現実的な」ユースケースを想定して作成されています。
...
- `reply`: 呼び出し元への返信メッセージを設定 (`[message]`)。
...
[
  ...
  {
    "op": "reply",
    "args": ["初回ボーナスとして 10.000 コインを送金しました！"]
  }
]
```
...
[
  ...
  },
  {
    ...
    "then": [
      ...
      {
        "op": "reply",
        "args": ["おめでとうございます！当たりです！5.000コインを獲得しました。"]
      }
    ],
    "else": [
      {
...
[
  ...
  },
  {
    ...
    "else": [
      {
        "op": "cancel",
        "args": ["金額が不足しています。10.000コイン以上必要です。"]
      }
    ]
  },
  {
...
- `discord_role_add` で外部への作用（ロール付与）を行っています。
